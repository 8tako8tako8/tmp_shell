...
void    del_dot_dot_slash(char **path, int i)
{
    ...
    while (tmp[j] != '/')
        j--;
    if (!(*path = malloc(ft_strlen(tmp))))//ft_strlen(tmp)-(i-j+1)だとバグ発生なぜ？
        exit(1);
    ft_strncpy(*path, tmp, j + 1);
    if (*(tmp + i + 3) != '\0')
    {
        ft_strlcpy(*path + j + 1, tmp + i + 3, ft_strlen(tmp));
        (*path)[j + ft_strlen(tmp + i + 3) + 1] = '\0';
    }
    free(tmp);
}
...
{
    char    *pwd = "/shell/cd";
    //char    *r_path = "../test/./test2/./test3/../../test4/";
    char    *r_path = "./test/.././test/.";
    char    *result;

    result = get_path_after_moving(pwd, r_path);
