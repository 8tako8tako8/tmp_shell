...
int    exist_file(char *path)
{
    ...
    {
        if ((st.st_mode & S_IXUSR) != S_IXUSR || stat(path, &st) == -1 || S_ISDIR(st.st_mode) || (st.st_mode & S_IXUSR) != S_IXUSR)
            return (0);
        return (1);
    }
    else if (S_ISREG(st.st_mode) && (st.st_mode & S_IXUSR) == S_IXUSR)
        return (1);
    return (0);
}
